...
public class BuggySearchAndSort {
	...
	public static boolean contains(int[] array, int val) {
		for (int i = 0; i < array.length -1; i++) { //dead code asking to remove the i++ but was not the probelm
			if (array[i] == val)
				return true;
			//else This else is desnecessary, this was making the program make one wrong step
				//return false;
		}
		return false;
	}
	...
	public static void bubbleSort(int[] array) {
		...
		for (int i = 0; i < array.length; i++) {
			for (int j = 0; j < array.length-1; j++) { // i++ in a wrong place then j will be always 0
	...
	public static void selectionSort(int[] array) {
		for (int top = array.length - 1; top > 0; top--) {
			int positionOfMax = 0;
			for (int i = 1; i <= top; i++) {
				if (array[i] > array[positionOfMax])  // this 1 is suspect
					positionOfMax = i;
			}
	...
	public static void insertionSort(int[] array) {
		for (int top = 1; top < array.length; top++) {
			...
			int pos = top - 1;
			while (pos >= 0 && array[pos] > temp) {                       // top is 0 error
				   // move items that are bigger than temp up one position
				array[pos+1] = array[pos];
				//System.out.print(" . " + array[pos+1]);  // to debbuging test
				pos--;
			}
			array[pos + 1 ] = temp;  // place temp into last vacated position
			//the lack of the + 1 the array was receiving the value without change its position
			//System.out.print(" . " + array[pos]);  // to debugging test
		}
	}
